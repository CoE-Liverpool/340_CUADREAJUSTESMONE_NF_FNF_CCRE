...
##    nfile='R17 MONETARY PREEDIT REPORT.TXT'
    ...
    if os.path.exists(path+"\\Temporal\\"+nfile) and nfile!='':
        ...
        with open(path+"\\Temporal\\"+nfile, 'r') as file:
            count=0
            ca=0
            cb=0
            vec=[]
    ...
    ##                print(li)
        ...
        nfile1 = path+"\\Temporal\\"+nfile.replace('.txt','')+'.xlsx'
        print(nfile1)
...
def main():
    ...
    try:
        ...
        print('Error on Line '+str(format(sys.exc_info()[-1].tb_lineno))+' <'+str(e)+'>')
        return('Error on Line '+str(format(sys.exc_info()[-1].tb_lineno))+' <'+str(e)+'>')
